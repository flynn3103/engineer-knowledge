...
1. You need 10,000 globally-linearizable writes/sec to a single key across three continents (~150 ms inter-region RTT). Feasible?
2. A PM says storing an activity feed is "too expensive." 1M users, 5 events/day each, ~200 bytes/event. Respond.
3. When is reasoning from fundamentals actively the wrong tool?
