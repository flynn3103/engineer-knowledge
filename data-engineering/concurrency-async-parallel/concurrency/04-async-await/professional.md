...
|---|---|---|
| **Shared-memory + locks** | Threads share memory directly, mutexes/atomics coordinate access | [Shared-Memory Concurrency](../01-models/01-shared-memory/README.md) |
| **Message passing / CSP** | No shared state; explicit `send`/`recv` of values through channels | [Message Passing](../01-models/02-message-passing/README.md), [CSP](../01-models/04-csp/README.md) |
| **Actor model** | Isolated actors, each with a private mailbox, communicating via async messages | [Actor Model](../01-models/03-actor-model/README.md) |
...
- Go's own guidance ("share memory by communicating") and CSP's original
  formal model (Hoare, 1978) — see [CSP](../01-models/04-csp/professional.md)
  for the full professional-level treatment.
- See also: [Actor Model — professional](../01-models/03-actor-model/professional.md),
