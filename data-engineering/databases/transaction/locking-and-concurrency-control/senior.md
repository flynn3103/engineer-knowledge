...
| Correctness-critical invariant that must never be violated even under bursty contention | Pessimistic, or Serializable isolation (see [Isolation Levels — senior](../isolation-levels/senior.md)) |
| Distributed / cross-service "lock" (no single database transaction spans it) | Optimistic-style version checks are usually the only practical option — see [Leases and Fencing](../../distributed-system/18-concurrency-coordination/leases-and-fencing/README.md) |
