...
|---|---|---|
| Mechanism | Acquire a distributed lock (via etcd/ZooKeeper — see [Coordination Services](../05-coordination-services/README.md)) before touching the resource | Read the resource's current version, act, then write conditionally on that version still being current (a conditional/compare-and-swap write) |
| Failure mode if wrong | A crashed lock-holder can block everyone else — needs a lease (see [Leases & Fencing](../02-leases-and-fencing/README.md)) | A conflicting concurrent write causes your write to be rejected — you must retry |
