...
> Which system invariant is affected by **`go mod tidy`** under failure, load, and change?
...
1. State the system invariant that **`go mod tidy`** must protect.
...
- Which invariant must remain true when `go mod tidy` fails?
