...
> Where does **`go mod init`** belong in a maintainable component, and which trade-off selects the design?
...
1. Find a real component where **`go mod init`** affects an interface or dependency.
...
- Which boundary is most affected by `go mod init`?
