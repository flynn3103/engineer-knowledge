...
> Which system invariant is affected by **`go mod init`** under failure, load, and change?
...
1. State the system invariant that **`go mod init`** must protect.
...
- Which invariant must remain true when `go mod init` fails?
