...
After this file, the natural progression is to read `specification.md` for the numeric metrics (change coupling thresholds, bidirectional change count, tool calibration), then `find-bug.md` for ten concrete Java scenarios where shotgun surgery hides, then `optimize.md` for the CI and JIT cost angle. The `tasks.md` exercises will let you run `CodeMaat` against a real repo and feel the coupling graph in your own codebase. After this cluster, the next code smell - Feature Envy (file [`../03-feature-envy/`](../03-feature-envy/)) - is the natural counterpart: shotgun surgery is one change across many files; feature envy is one file reaching into many others. The closely related [`../05-inappropriate-intimacy/`](../05-inappropriate-intimacy/) (classes that reach into each other's internals) and [`../08-data-clumps/`](../08-data-clumps/) (the same field group scattered everywhere — a frequent shotgun trigger) round out the set. All four are coupling smells, and all four trace back to one design principle viewed from different angles: the Single Responsibility Principle ([`../../03-design-principles/01-solid-principles/`](../../03-design-principles/01-solid-principles/)). Shotgun Surgery is the SRP failure where a single responsibility is *smeared* across many classes; Divergent Change is the SRP failure where many responsibilities are *crammed* into one. Reading them as a pair makes both clearer.
