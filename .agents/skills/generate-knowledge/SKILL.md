...
1. **Research the topic on the web before writing anything.** Fetch 2-4 real sources
   ...
   common failure modes if one exists). Do not synthesize purely from memory —
   the whole point of this format is that content is grounded in citable material.
...
- **Ground every non-obvious claim in what you actually fetched.** If research
  surfaced a named result (e.g. a documented principle, a named rule of thumb,
  a quote from a primary source), use it — that's the difference between this
...
| Writing in academic or corporate vocabulary that assumes prior expertise | Rewrite using everyday words; define any unavoidable term once, in plain language, the first time it's used |
