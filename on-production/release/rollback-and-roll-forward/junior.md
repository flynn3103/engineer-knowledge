...
> **Rule of thumb: roll back to an artifact, not to a commit.** Pull the known-good image from the registry by its immutable tag or digest and deploy that. This is why immutable, retained artifacts in a registry matter — see [Registries & Distribution](../registries-and-distribution/middle.md).
...
- Wrapping a risky change in a flag gives you an instant "undo" that doesn't depend on `kubectl` or rebuilds at all. Go deep in [Feature Flags & Progressive Delivery](../feature-flags-and-progressive-delivery/junior.md).
...
2. **Tag releases meaningfully** so the version in `rollout history` maps to a real, findable artifact in your registry. See [Versioning & SemVer](../versioning-and-semver/junior.md).
