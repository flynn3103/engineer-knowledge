...
2. **Sign artifacts and verify on pull/admission.** Only signed-by-us images are admitted. This is the link to [Artifact Signing & Provenance](../artifact-signing-and-provenance/senior.md): the registry stores the artifact and its signature; the *admission gate* enforces "no signature, no run."
...
> Senior principle: the registry is where you *enforce* supply-chain policy because it's the one chokepoint every artifact passes through. Detail lives in [Supply-Chain Security](../supply-chain-security/senior.md).
