...
4. **Have a documented manual runbook** for the cases automation can't fix — e.g. a publish that *partially* uploaded a multi-arch image. The runbook says exactly which commands to run to complete or roll back. (Rollback specifics live in [Rollback and Roll-forward](../rollback-and-roll-forward/README.md).)
...
> Notice the verification pins *which workflow* may have signed. That is the payoff: the signature attests not just "a valid key" but "produced by this org's release workflow." Combined with SLSA provenance, you get a verifiable claim about exactly how the artifact was built. Full detail in [Artifact Signing and Provenance](../artifact-signing-and-provenance/README.md) and [Supply-chain Security](../supply-chain-security/README.md).
...
> Channel design connects directly to [Release Branching and Trains](../release-branching-and-trains/README.md): your branch model *is* your channel model. Decide them together.
