...
- Owns the rollback decision if GA goes wrong (with [Rollback & Roll-Forward](../rollback-and-roll-forward/README.md) procedures).
...
- The **runbook** turns the RM role into something a rotation can execute without tribal knowledge: a step-by-step for cutting the branch, building/promoting RCs, the RRR checklist, the exception procedure, the GA steps, and the rollback procedure.
  ...
  - Automate every step the runbook describes that doesn't require judgment ([Release Automation](../release-automation/README.md)).
...
The strategic point: in a mature org the *long-lived release branch is largely replaced by runtime flag control*, which is why flag governance is now part of release engineering, not a separate concern. (See [Feature Flags & Progressive Delivery](../feature-flags-and-progressive-delivery/README.md).)
