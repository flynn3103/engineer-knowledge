...
- **An audit-log reconstruction.** Given only the audit trail (see [Audit Logging](../04-audit-logging/README.md)), can you answer "was this specific user's DEK actually destroyed, and when" without trusting a status flag in an application database that could itself be wrong?
