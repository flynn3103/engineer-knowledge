...
| **Crypto-shredding** | Data stays encrypted; the per-subject key is destroyed | Cold storage, backups, or data lakes where per-record deletion is impractical | Requires per-subject (or per-tenant) encryption keys designed in *before* you need to shred them — see [Encryption Key Lifecycle](../05-encryption-key-lifecycle/README.md) |
