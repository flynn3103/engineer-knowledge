# Authentication — Junior

<!-- level-focus -->
At junior level, focus on this capability:

> What is the smallest safe way to prove the caller’s identity before granting access?

## Mental model

perform prove the caller’s identity before granting access in a small, defined system. In this module, the important vocabulary is **credentials, sessions, and multifactor verification**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Implement one explicit control for an account service. Name the protected asset, the trusted input, and the expected deny behavior.
...
Create a small an account service with a test identity. Apply the rule, then capture the HTTP status, audit event, or command output for both the allowed and denied cases.
