# DDoS Mitigation — Junior

<!-- level-focus -->
At junior level, focus on this capability:

> What is the smallest safe way to keep legitimate traffic working during a volumetric or application-layer attack?

## Mental model

perform keep legitimate traffic working during a volumetric or application-layer attack in a small, defined system. In this module, the important vocabulary is **capacity, filtering, and incident controls**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Implement one explicit control for a public API. Name the protected asset, the trusted input, and the expected deny behavior.
...
For a public API, write a one-page decision record: asset, actor, trust boundary, rule, failure behavior, owner, and evidence source. Use that record to keep implementation and operations aligned.

## Apply it

Create a small a public API with a test identity. Apply the rule, then capture the HTTP status, audit event, or command output for both the allowed and denied cases.
