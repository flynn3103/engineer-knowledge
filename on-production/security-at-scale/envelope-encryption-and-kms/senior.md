# Envelope Encryption and KMS — Senior
...
design use a key-management service without exposing a master key to application data paths across components under failure and change. In this module, the important vocabulary is **data-encryption keys, key-encryption keys, and rotation**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.
