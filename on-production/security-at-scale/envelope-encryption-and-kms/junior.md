# Envelope Encryption and KMS — Junior

<!-- level-focus -->
At junior level, focus on this capability:

> What is the smallest safe way to use a key-management service without exposing a master key to application data paths?

## Mental model

perform use a key-management service without exposing a master key to application data paths in a small, defined system. In this module, the important vocabulary is **data-encryption keys, key-encryption keys, and rotation**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Implement one explicit control for an object-storage service. Name the protected asset, the trusted input, and the expected deny behavior.
...
For an object-storage service, write a one-page decision record: asset, actor, trust boundary, rule, failure behavior, owner, and evidence source. Use that record to keep implementation and operations aligned.

## Apply it

Create a small an object-storage service with a test identity. Apply the rule, then capture the HTTP status, audit event, or command output for both the allowed and denied cases.
