# OAuth 2.0 and OpenID Connect — Junior

<!-- level-focus -->
At junior level, focus on this capability:

> What is the smallest safe way to delegate access and obtain identity claims without sharing passwords?

## Mental model

perform delegate access and obtain identity claims without sharing passwords in a small, defined system. In this module, the important vocabulary is **authorization code flow, scopes, and ID tokens**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Implement one explicit control for a web application. Name the protected asset, the trusted input, and the expected deny behavior.
...
For a web application, write a one-page decision record: asset, actor, trust boundary, rule, failure behavior, owner, and evidence source. Use that record to keep implementation and operations aligned.

## Apply it

Create a small a web application with a test identity. Apply the rule, then capture the HTTP status, audit event, or command output for both the allowed and denied cases.
