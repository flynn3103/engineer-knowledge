# OAuth 2.0 and OpenID Connect — Professional
...
lead organization-scale adoption of delegate access and obtain identity claims without sharing passwords. In this module, the important vocabulary is **authorization code flow, scopes, and ID tokens**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Treat OAuth 2.0 and OpenID Connect as a product: publish a supported interface, default-safe templates, ownership boundaries, service objectives, and a path for exceptions with expiry.
