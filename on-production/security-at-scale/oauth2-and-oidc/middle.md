# OAuth 2.0 and OpenID Connect — Middle
...
choose maintainable boundaries for delegate access and obtain identity claims without sharing passwords in a real codebase. In this module, the important vocabulary is **authorization code flow, scopes, and ID tokens**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.
