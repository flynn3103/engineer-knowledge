# OAuth 2.0 and OpenID Connect — Senior
...
design delegate access and obtain identity claims without sharing passwords across components under failure and change. In this module, the important vocabulary is **authorization code flow, scopes, and ID tokens**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.
