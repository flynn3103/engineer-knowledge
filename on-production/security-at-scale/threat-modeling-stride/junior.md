# Threat Modeling with STRIDE — Junior

<!-- level-focus -->
At junior level, focus on this capability:

> What is the smallest safe way to find and prioritize security threats before implementation?

## Mental model

perform find and prioritize security threats before implementation in a small, defined system. In this module, the important vocabulary is **assets, trust boundaries, and STRIDE categories**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Implement one explicit control for a new payment workflow. Name the protected asset, the trusted input, and the expected deny behavior.
...
For a new payment workflow, write a one-page decision record: asset, actor, trust boundary, rule, failure behavior, owner, and evidence source. Use that record to keep implementation and operations aligned.

## Apply it

Create a small a new payment workflow with a test identity. Apply the rule, then capture the HTTP status, audit event, or command output for both the allowed and denied cases.
