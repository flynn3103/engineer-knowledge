# WAF and API Security — Professional
...
lead organization-scale adoption of apply edge protections without treating them as a substitute for secure application code. In this module, the important vocabulary is **request validation, WAF rules, and API gateways**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Treat WAF and API Security as a product: publish a supported interface, default-safe templates, ownership boundaries, service objectives, and a path for exceptions with expiry.
