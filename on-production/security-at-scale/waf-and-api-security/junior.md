# WAF and API Security — Junior

<!-- level-focus -->
At junior level, focus on this capability:

> What is the smallest safe way to apply edge protections without treating them as a substitute for secure application code?

## Mental model

perform apply edge protections without treating them as a substitute for secure application code in a small, defined system. In this module, the important vocabulary is **request validation, WAF rules, and API gateways**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Implement one explicit control for a public checkout API. Name the protected asset, the trusted input, and the expected deny behavior.
...
For a public checkout API, write a one-page decision record: asset, actor, trust boundary, rule, failure behavior, owner, and evidence source. Use that record to keep implementation and operations aligned.

## Apply it

Create a small a public checkout API with a test identity. Apply the rule, then capture the HTTP status, audit event, or command output for both the allowed and denied cases.
