# Threat Modeling with STRIDE — Professional
...
lead organization-scale adoption of find and prioritize security threats before implementation. In this module, the important vocabulary is **assets, trust boundaries, and STRIDE categories**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Treat Threat Modeling with STRIDE as a product: publish a supported interface, default-safe templates, ownership boundaries, service objectives, and a path for exceptions with expiry.
