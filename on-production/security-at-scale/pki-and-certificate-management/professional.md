# PKI and Certificate Management — Professional
...
lead organization-scale adoption of operate certificates as expiring production dependencies. In this module, the important vocabulary is **certificate authorities, issuance, renewal, and trust stores**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Treat PKI and Certificate Management as a product: publish a supported interface, default-safe templates, ownership boundaries, service objectives, and a path for exceptions with expiry.
