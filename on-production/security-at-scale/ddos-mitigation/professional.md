...
lead organization-scale adoption of keep legitimate traffic working during a volumetric or application-layer attack. In this module, the important vocabulary is **capacity, filtering, and incident controls**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Treat DDoS Mitigation as a product: publish a supported interface, default-safe templates, ownership boundaries, service objectives, and a path for exceptions with expiry.
