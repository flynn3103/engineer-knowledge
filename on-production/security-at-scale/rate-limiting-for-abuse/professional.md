# Rate Limiting for Abuse — Professional
...
lead organization-scale adoption of bound abusive or accidental demand fairly and predictably. In this module, the important vocabulary is **quotas, token buckets, and identity keys**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Treat Rate Limiting for Abuse as a product: publish a supported interface, default-safe templates, ownership boundaries, service objectives, and a path for exceptions with expiry.
