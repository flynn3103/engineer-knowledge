# Encryption at Rest and Transit — Professional
...
lead organization-scale adoption of protect data while it travels and while it is stored. In this module, the important vocabulary is **TLS, encryption keys, and authenticated encryption**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Treat Encryption at Rest and Transit as a product: publish a supported interface, default-safe templates, ownership boundaries, service objectives, and a path for exceptions with expiry.
