# Encryption at Rest and Transit — Junior

<!-- level-focus -->
At junior level, focus on this capability:

> What is the smallest safe way to protect data while it travels and while it is stored?

## Mental model

perform protect data while it travels and while it is stored in a small, defined system. In this module, the important vocabulary is **TLS, encryption keys, and authenticated encryption**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Implement one explicit control for a customer record service. Name the protected asset, the trusted input, and the expected deny behavior.
...
For a customer record service, write a one-page decision record: asset, actor, trust boundary, rule, failure behavior, owner, and evidence source. Use that record to keep implementation and operations aligned.

## Apply it

Create a small a customer record service with a test identity. Apply the rule, then capture the HTTP status, audit event, or command output for both the allowed and denied cases.
