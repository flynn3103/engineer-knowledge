# Envelope Encryption and KMS — Middle
...
choose maintainable boundaries for use a key-management service without exposing a master key to application data paths in a real codebase. In this module, the important vocabulary is **data-encryption keys, key-encryption keys, and rotation**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.
