# Envelope Encryption and KMS — Professional
...
lead organization-scale adoption of use a key-management service without exposing a master key to application data paths. In this module, the important vocabulary is **data-encryption keys, key-encryption keys, and rotation**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Treat Envelope Encryption and KMS as a product: publish a supported interface, default-safe templates, ownership boundaries, service objectives, and a path for exceptions with expiry.
