# Secrets Management — Junior

<!-- level-focus -->
At junior level, focus on this capability:

> What is the smallest safe way to keep passwords, keys, and tokens out of code and rotate them safely?

## Mental model

perform keep passwords, keys, and tokens out of code and rotate them safely in a small, defined system. In this module, the important vocabulary is **secret stores, least privilege, and rotation**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Implement one explicit control for a deployment pipeline. Name the protected asset, the trusted input, and the expected deny behavior.
...
For a deployment pipeline, write a one-page decision record: asset, actor, trust boundary, rule, failure behavior, owner, and evidence source. Use that record to keep implementation and operations aligned.

## Apply it

Create a small a deployment pipeline with a test identity. Apply the rule, then capture the HTTP status, audit event, or command output for both the allowed and denied cases.
