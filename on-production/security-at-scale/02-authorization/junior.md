...
> What is the smallest safe way to decide what an authenticated identity may do?

## Mental model

perform decide what an authenticated identity may do in a small, defined system. In this module, the important vocabulary is **permissions, roles, and policy checks**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Implement one explicit control for a document API. Name the protected asset, the trusted input, and the expected deny behavior.
...
For a document API, write a one-page decision record: asset, actor, trust boundary, rule, failure behavior, owner, and evidence source. Use that record to keep implementation and operations aligned.

## Apply it

Create a small a document API with a test identity. Apply the rule, then capture the HTTP status, audit event, or command output for both the allowed and denied cases.
