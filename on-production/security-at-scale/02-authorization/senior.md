...
design decide what an authenticated identity may do across components under failure and change. In this module, the important vocabulary is **permissions, roles, and policy checks**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.
