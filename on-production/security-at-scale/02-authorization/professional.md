...
lead organization-scale adoption of decide what an authenticated identity may do. In this module, the important vocabulary is **permissions, roles, and policy checks**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Treat Authorization as a product: publish a supported interface, default-safe templates, ownership boundaries, service objectives, and a path for exceptions with expiry.
