# JWT and Tokens — Junior

<!-- level-focus -->
At junior level, focus on this capability:

> What is the smallest safe way to issue and validate bearer credentials with bounded lifetime and audience?

## Mental model

perform issue and validate bearer credentials with bounded lifetime and audience in a small, defined system. In this module, the important vocabulary is **claims, signatures, expiry, and revocation**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Implement one explicit control for a service API. Name the protected asset, the trusted input, and the expected deny behavior.
...
Create a small a service API with a test identity. Apply the rule, then capture the HTTP status, audit event, or command output for both the allowed and denied cases.
