# DevSecOps and Supply Chain Security — Professional
...
lead organization-scale adoption of make secure delivery repeatable from source to deployed artifact. In this module, the important vocabulary is **dependency provenance, scanning, and signed builds**. Security work is useful only when the system makes the intended decision reliably and produces evidence that it did so.

Treat DevSecOps and Supply Chain Security as a product: publish a supported interface, default-safe templates, ownership boundaries, service objectives, and a path for exceptions with expiry.
