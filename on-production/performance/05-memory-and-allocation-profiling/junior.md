...
These two numbers get conflated constantly. Untangling them is the single most useful thing on this page.

- **Residency** (heap size, live set) is a *snapshot*: how much memory is alive right now — the water level in a tank.
- **Allocation rate** is a *flow*: how fast new memory is requested over time — the rate water pours in, and drains out.
- A tank can sit at a stable, low level while a firehose pours in and an equal drain pours out. Level (residency) stays small; *flow* (allocation rate) is enormous. That is the setup that quietly kills performance, because the drain — the garbage collector — works flat out the whole time.
...
Concrete example — low residency, high allocation rate:
...
- At no point does this hold more than a few kilobytes (low residency).
- It allocates **4 GB total** across the loop (a million × 4 KB) — all garbage the GC must chase.
- The fix isn't "use less memory at once"; it's "stop *churning*": allocate the buffer once, outside the loop, and reuse it.
...
Not all memory is equal. Two places your data can live — the difference is *free* vs *not free*.

- **The stack** is per-function scratch space.
  - When a function is called it gets a frame; when it returns, the frame vanishes instantly, automatically, at zero cost.
  - No GC is ever involved. Stack memory is the fastest memory there is.
- **The heap** is for things that must outlive the function that created them — a value you return a pointer to, something stored in a long-lived map, anything whose lifetime the compiler can't predict.
  - Heap memory is managed by the GC, and *every* heap allocation is work: now, to allocate it, and later, to collect it.
- The compiler decides which place each value goes using **escape analysis**: "does this value *escape* the function?"
  - If it can prove the value never outlives the function → stays on the stack (free).
  - If the value escapes — via a returned pointer, a closure, an interface — it must go on the heap.
...
- That `escapes to heap` line tells you exactly where an allocation will happen.
- Java and Python don't expose this as cleanly — in Python, essentially *everything* is a heap object — but the principle is universal: values that escape their creating scope cost more.
...
A one-paragraph cost model of the GC — not the algorithm (that's [Language Internals](../../../language-internals/)), just what it costs you.

- The collector's job: periodically scan the heap, find every object still reachable, and reclaim everything else so memory can be handed out again.
- To do this safely it needs to know nothing is changing underneath it, so it competes with — and sometimes briefly **pauses** — your program.
- The more you allocate, the sooner the heap fills its trigger threshold, the more often the collector runs, the more total CPU it spends, and the more your program stalls.
...
- Go's GC aims to keep overhead modest, but a churn-heavy service can easily spend **20–30% of its CPU** just collecting garbage.
- That's a third of your machine doing nothing but cleaning up after allocations you didn't need to make.
- Cut the allocation rate in half and you can hand that CPU back to the work that matters — often a *bigger* win than any algorithmic micro-tweak.
- The causal chain is the whole point: **you don't optimize the GC, you optimize the input to the GC.** You can't make collection cheaper, but you can give it less to collect.
...
Most allocation waste comes from a handful of patterns. Spot these and you'll fix the majority of real-world churn without any cleverness.
...
- The same trap exists everywhere: Java `StringBuilder` instead of `+=` in a loop; Python `"".join(parts)` instead of `s += p`.

**4. Avoid unnecessary copies.**
- Passing a large struct *by value* copies it.
- Passing a slice already shares its backing array (cheap), but `append`-ing to a shared slice can quietly allocate.
- Pass big things by pointer/reference when you don't need a copy.
...
Compare the bad string-concat version against the `strings.Builder` version:
...
- 7 allocs/op → 2 allocs/op, and the time dropped ~4.7×.
- You didn't *guess* it was faster — you measured fewer allocations and confirmed the speedup.
- That's the loop: change code, rerun `-benchmem`, watch `allocs/op` fall.

For *where in a running program* the allocations come from — heap profiles, flame graphs, the actual `pprof` mechanics — see [01 — Profiling](../01-profiling/01-cpu-profiling/junior.md). That topic owns "how to capture"; this one owns "what the numbers mean and how to bring them down." The junior rhythm: benchmark first to quantify, profile only when you need to *locate*.
...
**1. The log line that doubled GC load.**
- A service logs one structured line per request via `fmt.Sprintf("%s %d %v", ...)` plus a few string concatenations — about 12 allocations per request.
- At 5,000 requests/second that's 60,000 allocations/second of pure churn, and GC overhead sat near 28% of CPU.
- Switching to a zero-allocation logger that writes directly into a reused buffer dropped it to ~1 alloc/request; GC overhead fell to under 8%, and p99 latency improved from fewer pauses.
- The *residency* barely moved — the win was entirely in allocation *rate*.

**2. The slice that grew the hard way.**
- A data-import job built a result slice with `append` starting from `nil`, for ~2 million rows.
- Because the slice doubled its backing array as it grew, it reallocated and copied roughly 21 times, touching gigabytes of throwaway memory.
- One line — `make([]Row, 0, expectedRows)` — removed every one of those reallocations.
- Import time dropped 35% with no algorithm change at all.

**3. The "small heap, slow service" puzzle.**
- A team chased a latency problem assuming a memory leak, because the service "felt heavy." The heap graph was flat and low — no leak.
- The real culprit was allocation rate: a hot JSON-parsing path allocated a fresh map per message.
- A heap *profile* (captured per [01 — Profiling](../01-profiling/01-cpu-profiling/junior.md)) pointed at the exact line; reusing a pooled map collapsed the allocation rate and the latency followed.
- Lesson: low residency hid a high-churn problem in plain sight.
...
- Why does allocation rate matter for performance even when the heap stays small?
- What is the difference between a stack allocation and a heap allocation, and why is the stack considered "free"?
- What is escape analysis, and what makes a value escape to the heap?
- Why does putting a value into an interface (or `any`) often cause an allocation?
- What is the difference between allocation rate and heap residency?
- Why might a flat, stable memory graph still hide a performance problem?
