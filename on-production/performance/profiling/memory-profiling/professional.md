...
- `-inuse_space` is the retention view (what's alive now); `-inuse_objects` counts live objects (catches "millions of tiny things"). These are the leak-hunting views. (`-alloc_space`/`-alloc_objects` are the *rate* side — that's [allocation profiling](../../memory-and-allocation-profiling/professional.md), not retention.)
...
6. **Confusing allocation rate with retention.** `-alloc_space` (Go) / a high allocation flame graph tells you what *churns*, not what *leaks*. For a leak you want `-inuse_space`/`-inuse_objects` and the *retained* size in a dominator tree. (Allocation-rate work is [05 — Memory and Allocation Profiling](../../memory-and-allocation-profiling/professional.md).)
