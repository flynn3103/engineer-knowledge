...
3. **Confusing `alloc_*` (churn) with `inuse_*` (retention).** Optimizing allocation rate won't fix a leak, and chasing retained objects won't lower GC frequency. Churn is this page; retention is [Memory Profiling](../memory-profiling/senior.md). They're different columns of the same profile for a reason.
