...
- **Axis 1 — `alloc_` vs `inuse_`.**
  ...
  - Allocation profiling is the `alloc_*` column — the *rate* of churn. `inuse_*` is retained-memory profiling, the subject of [memory-profiling](../memory-profiling/middle.md).
