...
- The CPU profile (see [02 — Benchmarking](../02-benchmarking-and-microbenchmarks/middle.md)) will only ever show you the 5 ms of *service* time — it is blind to the 45 ms of *wait*. This is the single most common reason a profiler says "the code is fast" while users say "the service is slow."
...
- **Concurrency** is the Little's Law lever: raise `L` (more workers/connections) and you raise the throughput `λ` you can sustain at a given latency `W`.
  - Only up to the point where a *shared resource* saturates — then adding concurrency just lengthens the queue and `W` climbs (see [06 — Concurrency and Contention](../06-concurrency-and-contention/middle.md)).
