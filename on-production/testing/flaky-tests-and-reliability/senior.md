...
- **Time.** Never call `time.Now()` / `Date.now()` directly in business logic. Inject a `Clock`. Tests pass a fixed or fake clock; no flakiness from boundaries, timezones, or DST. (See [Test Doubles](../test-doubles-mocks-fakes/senior.md).)
...
- **Per-test isolation.** Each test gets a fresh schema/namespace, or runs inside a transaction rolled back at teardown. Containerized ephemeral databases (Testcontainers) give every run a clean instance. (See [Integration Testing](../integration-testing/senior.md).)
...
- **Independent test data.** Each test creates exactly the data it needs and owns its lifecycle. (See [Test Data Management](../test-data-management/senior.md).)
