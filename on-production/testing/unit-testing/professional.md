...
The anti-pattern to police is the **ice-cream cone** (mostly E2E, few units): slow, flaky, expensive, and it fails to localize. But blind "everything is a unit" is its own failure when applied to glue code. The right ratio is a function of *where the risk lives* — logic vs wiring — and that's an architectural judgment, not a fixed number. Use [Integration](../integration-testing/README.md) and [Contract](../contract-testing/README.md) testing to cover the seams units leave open.
