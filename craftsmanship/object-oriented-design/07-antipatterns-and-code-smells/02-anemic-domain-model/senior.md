...
- **Methods that orchestrate multiple aggregates.** `account.transferTo(other, amount)` blurs the consistency boundary. Better: a domain service `Transfer.execute(from, to, amount)` — the one legitimate home for logic that belongs to no single entity or aggregate.
...
2. How would you apply the ideas around Anemic-or-rich is a *context* decision, not a *taste* decision, Tell-Don't-Ask in depth, The four contexts where anemic is justified in a realistic engineering change?
3. What failure mode or misuse should you look for, and what evidence would reveal it?
4. How would you validate a system-level decision about Anemic Domain Model under uncertainty?
