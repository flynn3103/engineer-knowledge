...
A responsibility table tells you *who*; a collaboration sketch tells you *who calls whom*. Use a compact sequence diagram so the interaction remains easy to revise.

```mermaid
sequenceDiagram
    participant Controller as BookingController
    participant Process as BookingProcess
    participant Booking
    participant Room
    participant Rates as RateCard
    participant Payment as PaymentGateway
    Controller->>Process: confirm(request)
    Process->>Booking: reserve(dates)
    Booking->>Room: is_free(dates)?
    Room-->>Booking: availability
    Booking->>Rates: price_for(stay)
    Rates-->>Booking: amount
    Process->>Payment: charge(amount)
    Payment-->>Process: payment result
    Process-->>Controller: confirmation
```
...
| Hands-on RDD refactoring katas                                              | [check your understanding](#check-your-understanding)        |
...
1. Explain Responsibility-Driven Design in your own words and name the problem it solves.
2. How would you apply the ideas around The "who knows what" exercise, Designing collaborations, Refactoring a god-class — a worked example in a realistic engineering change?
3. What failure mode or misuse should you look for, and what evidence would reveal it?
4. Which local design trade-off would make you choose or reject Responsibility-Driven Design in an existing codebase?
