...
## 6. Sequence walk-through patterns
...
sequenceDiagram
    ...
    participant Room
    Customer->>FrontDesk: book(request)
    FrontDesk->>Inventory: find(type, dates)
    Inventory->>Room: is_free(dates)?
    Room-->>Inventory: availability
    Inventory-->>FrontDesk: matching room
    FrontDesk-->>Customer: confirmation
```
...
```mermaid
sequenceDiagram
    actor Agent as ClaimsAgent
    participant Policy as ClaimPolicy
    participant Claim
    Agent->>Policy: approve(claim)?
    Policy->>Claim: amount()
    Claim-->>Policy: claimed amount
    Policy->>Claim: coverage()
    Claim-->>Policy: coverage terms
...
2. How would you apply the ideas around Multi-scenario walk-throughs — how a card set evolves, From cards to Java — responsibilities to signatures, Identifying god cards and splitting them mid-session in a realistic engineering change?
3. What failure mode or misuse should you look for, and what evidence would reveal it?
4. Which local design trade-off would make you choose or reject CRC Cards in an existing codebase?
