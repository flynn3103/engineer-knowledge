...
class Cart:
    ...
    def check_out(self, payment: PaymentMethod) -> Receipt: ...
```
...
```python
class FuelPump:
    def begin_session(self, customer: Customer, prepaid: Money) -> Session: ...
    def dispense(self, requested: Liters) -> None: ...
    def end_session(self) -> Receipt: ...
    def report_fault(self, code: FaultCode) -> None: ...
```
...
```python
class FuelPump:
    def __init__(self, price_per_liter: Money) -> None:
        self._current: Session | None = None  # needed by begin/end session
        self._price_per_liter = price_per_liter  # needed to price dispensing
        self._faulted = False  # changed only by report_fault
...
class User:
    ...
    # Public mutation of every attribute would leave rules to callers.
```
...
2. How would you apply the ideas around Two mental models, two outcomes, The shift in your head, A worked example — a fuel pump in a realistic engineering change?
3. What failure mode or misuse should you look for, and what evidence would reveal it?
4. What small example would prove that you can apply Behavior-First Mindset correctly?
