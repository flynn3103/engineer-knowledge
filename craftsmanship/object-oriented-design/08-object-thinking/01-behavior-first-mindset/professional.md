...
```python
class Cancellation:
    """Behavior-first seam around a legacy persistence record."""

    def __init__(self, row: OrderRow, policy: RefundPolicy, clock: Clock) -> None:
        self._row = row
        self._policy = policy
        self._clock = clock

    def cancel(self, reason: Reason) -> Refund:
        if self._row.status == OrderStatus.SHIPPED:
            raise ValueError("shipped orders cannot be cancelled")
        return self._policy.refund_for(self._row, reason, self._clock.now())
```
...
```python
class Order:
    def __init__(self, order_id: OrderId, repo: CustomerRepository,
                 email: EmailService) -> None:
        self.customer = repo.by_id(order_id)  # I/O in a constructor: avoid
        email.send_welcome(self.customer)     # Hidden side effect: avoid
```
...
def test_domain_does_not_import_persistence() -> None:
    domain_source = "\n".join(
        path.read_text() for path in Path("src/domain").rglob("*.py")
    )
    assert "from infrastructure" not in domain_source
    assert "import sqlalchemy" not in domain_source

def test_domain_has_no_public_setters() -> None:
    public_setters = [
        line for path in Path("src/domain").rglob("*.py")
        for line in path.read_text().splitlines()
        if line.lstrip().startswith("def set_")
    ]
    assert public_setters == []
```
...
2. How would you apply the ideas around Set the vocabulary before you set the rule, Code review dialog patterns, Spotting at scale — the grep pass in a realistic engineering change?
3. What failure mode or misuse should you look for, and what evidence would reveal it?
4. How would you introduce and govern Behavior-First Mindset across teams through reversible, measurable increments?
