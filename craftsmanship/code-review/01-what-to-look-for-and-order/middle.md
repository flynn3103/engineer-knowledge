# What to Look For & Review Order — Middle

## Purpose

Explain the mechanism, not just the rule. This guide keeps the topic practical: review contracts, boundaries, state, failure paths, and maintainability in order.
...
- review contracts, boundaries, state, failure paths, and maintainability in order.
...
You should be able to diagnose common failures, choose a trade-off, and show evidence. Keep the loop simple: make the contract explicit, test the risky path, measure the result, and improve the system that produced it.
