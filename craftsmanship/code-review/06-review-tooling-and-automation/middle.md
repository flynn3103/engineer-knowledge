# Review Tooling & Automation — Middle
...
- **CI check** — an automated job (lint, type-check, test, SAST) run on the PR by the CI system, reported as a pass/fail status that a quality gate can require.
- **SAST** — Static Application Security Testing; security-focused static analysis run as a check (e.g. CodeQL, Semgrep).
...
2. **Adding checks without making them gates.** A linter that runs but doesn't *fail* the PR is advisory, and advisory checks get ignored. If a category matters, make the check block merge (Quality Gates); if it doesn't matter enough to block, ask why it runs at all.
...
- Static Analysis & Linting — Layer 2 in depth: the linters, type-checkers, and SAST tools that run before a human looks.
- Quality Gates — the policy layer that turns a CI check into a *required* gate and CODEOWNERS approval into a *mandatory* one.
- Testing — Layer 3: the test suite and coverage signals the bots post on the PR.

---

## Check your understanding

1. Explain Review Tooling & Automation — Middle Level in your own words and name the problem it solves.
2. How would you apply the ideas around Table of Contents, Introduction, Prerequisites in a realistic engineering change?
3. What failure mode or misuse should you look for, and what evidence would reveal it?
4. Which local design trade-off would make you choose or reject Review Tooling & Automation — Middle Level in an existing codebase?
