...
**Hints.**
- Agree the label contract *first*, as a shared doc, then implement per language — a single key mismatch (`service` vs `service_name`) breaks the uniform query, exactly like the cardinality lesson in `../04-metrics/`.
...
- [Continuous Profiling — Interview](interview.md)
